create or replace function public.quote_checkout_v2(
  p_store_id uuid,
  p_items jsonb,
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    if exists(select 1 from public.product_variants where product_id=v_product_id and active=true) then
  ...
  return query select * from public.quote_checkout(p_store_id,p_items,p_delivery_method,p_coupon_code);
end
$function$;
...
revoke all on function public.quote_checkout_v2(uuid,jsonb,text,text) from public, anon;
revoke all on function public.create_checkout_order_v2(uuid,jsonb,uuid,text,text,text) from public, anon;
grant execute on function public.quote_checkout_v2(uuid,jsonb,text,text) to authenticated;
